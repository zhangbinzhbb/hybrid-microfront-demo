module.exports = {
  ...
  rules: {
    ...
    // *【==== 关闭配置 ====】* //
    'vue/no-unused-components': 0, // components 校验
    'vue/html-indent': 0, // 强制使用一致的缩进 <template>
    ...
    'vue/no-use-v-if-with-v-for': 0, // 防止在同一元素上同时使用v-for和v-if指令。
    'vue/valid-v-for': 0,
    camelcase: [0], //为属性强制执行驼峰命名
    'comma-dangle': [0], // 逗号不使用悬挂
    'consistent-this': [0], //强制this别名为that
    radix: 0, //parseInt必须指定第二个参数
    semi: [0], // 不使用分号
    'semi-spacing': [0], // 强制分号间
